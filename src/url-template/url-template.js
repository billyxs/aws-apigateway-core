...
// TODO: change since draft-0.6 about characters in literals
  /* extract: The characters outside of expressions in a URI Template string are
    intended to be copied literally to the URI-reference if the character is
    allowed in a URI (reserved / unreserved / pct-encoded) or, if not allowed,
    copied to the URI-reference in its UTF-8 pct-encoded form.*/
...
// var SELECTEDCHARS_RE = new RegExp("[]","g");
  ...
  Expression.build = function (ops, vars) {
    ...
    switch (ops) {
      ...
      default : throw new Error("Unexpected operator: '"+ops+"'")
    }
  ...
  VarSpec.build = function (name, expl, part, nums) {
    var valueHandler
    var explBool = !expl
    if (!explBool) { // interprete as boolean
  ...
  var match2expression = function (m) {
    ...
    for (i = 0; i<len; i++) {
      ...
      if ((match = vars[i].match(VARSPEC_RE)) === null) {
        throw new Error('unexpected parse error in varspec: ' + vars[i])
      }
